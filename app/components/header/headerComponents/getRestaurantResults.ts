...
const getRestaurantResults = async (searchQuery: string) => {
    ...
    const { data: fromProduct, error: productError } = await supabase
        ...
        .select()
        .ilike('restaurant_name', formattedQuery); // Use ilike for partial matching
    ...
    for (const product of fromProduct) {
        console.log("product")
        console.log(product)
...
export default getRestaurantResults;
