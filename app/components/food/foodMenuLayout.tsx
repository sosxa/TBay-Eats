...
const FoodDiv: React.FC<FoodDivProps> = ({ filter, prices, type, rdyToFetch }) => {
  ...
  const fetchData = useCallback(async (pageNumber: number) => {
    if (rdyToFetch) {
      try {
        setLoading(pageNumber === 1); // Set loading true only for the initial fetch
        setLoadingMore(pageNumber > 1); // Set loadingMore true for subsequent fetches
        const data = await fetchFilter(filter, [prices.min, prices.max], type, pageNumber);
        if (data.length < 9) {
          setHasMore(false); // No more products to load
        }
        setProducts((prev) => pageNumber === 1 ? data : [...prev, ...data]);
        setDisplayedProducts((prev) => pageNumber === 1 ? data.slice(0, 9) : [...prev, ...data.slice(0, 9)]);
      } catch (error) {
        console.error('Error fetching products:', error);
      } finally {
        setLoading(false);
  ...
  const handleAddToCartClick = (event: React.MouseEvent<HTMLButtonElement>, product: Product) => {
    ...
    const item = {
      ...
      ogPrice: isCombo ? product.price : (product.price_size?.[0]?.price || product.price),
      activeDiscount: product.active_discount,
      discountPrice: !isCombo && product.discount_price_size?.[0]?.price ? product.discount_price_size[0].price : product.active_discount ? product.discount_price_size?.[0]?.price : product.price,
      discountAmount: product.discount_amount,
      comboItems: isCombo ? product.combo_items : undefined,
      spiceLevel: isProduct ? product.spice_level?.[0]?.label : undefined,
      size: isProduct ? product.price_size?.[0]?.size : undefined
        ...
        >
          ...
          <div className='w-full pt-[1.5rem] pb-[0rem] px-[.5rem] flex items-center justify-between'>
            ...
            <h4 className='text-xl font-semibold'>
              {!loading && type === "products"
                ? `$${product.active_discount ? product.discount_price_size?.[0]?.price : product.price_size?.[0]?.price}`
                : `$${product.active_discount ? product.discount_price_size?.[0]?.price : product.price}`}
            </h4>
          </div>
          <div className='w-full px-[.5rem] flex gap-4 pb-[1rem]'>
            <div className='bg-gray-300 rounded-md w-[3.5rem] flex items-center justify-center px-5'>
              <div className='w-6 h-6 flex items-center justify-center'>
                <svg id="Layer_1" data-name="Layer 1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 122.88 117.19" className="w-4 h-4">
                  <path d="M64.39,2,80.11,38.76,120,42.33a3.2,3.2,0,0,1,1.83,5.59h0L91.64,74.25l8.92,39a3.2,3.2,0,0,1-4.87,3.4L61.44,96.19,27.09,116.73a3.2,3.2,0,0,1-4.76-3.46h0l8.92-39L1.09,47.92A3.2,3.2,0,0,1,3,42.32l39.74-3.56L58.49,2a3.2,3.2,0,0" />
                </svg>
              </div>
            </div>
          </div>
  ...
  return (
    <div className='w-full'>
      {loading ? renderSkeletonLoader() : renderProducts()}
      {hasMore && !loadingMore && (
      ...
      {asideOpen && selectedProduct && (
        <SlidingAside product={selectedProduct} onClose={handleCloseAside} />
      )}
    </div>
  );
};
