...
const RestaurantMenuLayout: React.FC<FoodDivProps> = ({ filter, prices, type, rdyToFetch, restaurantEmail, userId }) => {
    ...
    }, [rdyToFetch, restaurantEmail, userId, filter, prices, type, page]);

    useEffect(() => {
        // Reset the page and fetch data when filter, prices, or type changes
        setPage(1);
        fetchData();
    }, [filter, prices, type, fetchData]);
    ...
    return (
        <>
            {loading ? (
                ...
                <div className='w-full flex flex-wrap gap-2 px-3 sm:px-[4rem] md:px-5 md:gap-4 lg:px-2 md:grid md:grid-cols-2 lg:grid-cols-3'>
                    {products.map((product) => (
                        <div key={product.id} className='w-full rounded-xl shadow-lg bg-[#F9F6EE] transform transition-transform duration-300 ease-in-out hover:scale-105 cursor-pointer relative p-4' onClick={() => router.push('/product/' + product.id)}>
                            ...
                            <div className='w-full pt-[1.5rem] pb-[0rem] px-[.5rem] flex items-center justify-between'>
                                ...
                                <h4 className='text-xl font-semibold'>${product.active_discount ? product.discount_price : product.price}</h4>
                            </div>
                        </div>
                    ))}
                    {hasMore && (
